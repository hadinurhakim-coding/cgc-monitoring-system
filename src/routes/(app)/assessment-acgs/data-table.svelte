<script lang="ts">
  ...
  import {
    saveAssessmentField,
    uploadEvidenceWithSignedUrl
  } from "./_lib/assessment-api-client.js";
  import {
  ...
  /** Cocokkan status dari DB (yes/no/na, case-insensitive) dengan tombol YES/NO/NA. */
  function statusMatches(q: AssessmentItem, expected: "YES" | "NO" | "NA"): boolean {
  ...
  /** Jangan reset halaman saat data refresh setelah simpan — hanya saat tahun / filter URL berubah. */
  $effect(() => {
    currentYear;
    serverSearch;
    currentPage = 1;
  ...
  /** Simpan teks: Ctrl+Enter (Windows/Linux) atau ⌘+Enter (macOS). */
  async function handleKeyDown(e: KeyboardEvent, q: AssessmentItem, field: string) {
    if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
      e.preventDefault();
      await saveField(q, field, (e.currentTarget as HTMLTextAreaElement).value);
    }
  }
  ...
  async function saveField(q: AssessmentItem, field: string, value: string) {
    ...
    if (error) {
      ...
      await invalidate("assessment-acgs:data");
    }
  }
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                  ...
                  <td class="border border-border p-2 align-top">
                    <textarea
                      ...
                      placeholder="Implementasi — Ctrl+Enter atau ⌘+Enter untuk simpan"
                      value={q.implementation ?? ""}
                      oninput={(e) => {
                        q.implementation = e.currentTarget.value;
                      }}
                  ...
                  <td class="border border-border p-3 align-top">
                    ...
                    <div class="relative group">
                      <textarea
                        ...
                        value={q.evidence ?? ""}
                        oninput={(e) => {
                          q.evidence = e.currentTarget.value;
                        }}
                  ...
                  <td class="border border-border p-2 align-middle text-center">
                    <div class="flex flex-col gap-1 items-center">
                        <button
                            type="button"
                            class="w-10 py-1 rounded text-[9px] font-bold transition-all {statusMatches(q, 'YES') ? 'bg-emerald-500 text-white shadow-sm' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}"
                        ...
                        <button
                            type="button"
                            class="w-10 py-1 rounded text-[9px] font-bold transition-all {statusMatches(q, 'NO') ? 'bg-rose-500 text-white shadow-sm' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}"
                        ...
                        <button
                            type="button"
                            class="w-10 py-1 rounded text-[9px] font-bold transition-all {statusMatches(q, 'NA') ? 'bg-slate-400 text-white shadow-sm' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}"
                  ...
                  <td class="border border-border p-2 align-top">
                    <textarea
                      use:autoResize
                      class="w-full bg-slate-50 border border-slate-200 rounded p-2 min-h-[100px] text-[10px] focus:bg-white focus:ring-1 focus:ring-primary focus:border-primary transition-all resize-none overflow-hidden"
                      placeholder="Rekomendasi — Ctrl+Enter atau ⌘+Enter untuk simpan"
                      value={q.recommendation ?? ""}
                      oninput={(e) => {
                        q.recommendation = e.currentTarget.value;
                      }}
