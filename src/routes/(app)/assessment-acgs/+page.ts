...
export const load: PageLoad = async ({ url, fetch, parent }) => {
	...
	const cacheKey = createPageCacheKey({ ...scope, route: ROUTE, year, search });
	const dataUrl = `${resolve("/assessment-acgs/api/data")}?year=${year}&q=${encodeURIComponent(search)}`;
	const payload = await loadCachedPageData<AssessmentDataPayload>({
		fetcher: fetch,
		url: dataUrl,
		cacheKey,
		scope,
