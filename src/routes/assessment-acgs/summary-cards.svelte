  ...
  const stats = $derived.by(() => {
    ...
    if (total === 0) {
      return {
        ...
        compliance: { score: "0", trend: "N/A", status: "stable" as const },
        parts: [] as { id: string; label: string; color: string; score: number }[],
    ...
    const urgent = questions.filter(
      (q) =>
        ...
        (q.level_label || q.level)?.includes("LEVEL 1")
    ).length;

    return {
      progress: { total, completed, percentage: progressPerc },
      compliance: { score: complianceScore, trend: "+0%", status: "stable" as const },
      parts: partsStats,
