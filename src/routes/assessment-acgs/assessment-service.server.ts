...
} from "./assessment-metrics.server.js";
...
export async function getAssessmentPageData(
	...
	const emptyPayload = (): AssessmentPagePayload => ({
		...
		summaryMetrics: computeAssessmentSummaryMetrics([]),
		questions: [],
		questionsTotal: 0,
		questionsOffset: 0,
		questionsLimit: 0,
	...
	const questions = processQuestionRowsForClient(
		pageRes.data as FlatAssessmentRow[],
		subMap
	);

	const summaryRows = questions as unknown as QuestionSummaryRow[];

	return {
		availableYears: state.availableYears,
		summaryMetrics: computeAssessmentSummaryMetrics(summaryRows),
		questions,
