-- Security hardening: RLS, storage privacy, function privileges, and FK indexes.

-- ---------------------------------------------------------------------------
-- Shared updated_at trigger function: explicit search_path.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- public.acgs_assessments: authenticated users may read by role/division only.
-- Writes must go through server-side authorization using service role/RPC.
-- ---------------------------------------------------------------------------
ALTER TABLE public.acgs_assessments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.acgs_assessments FORCE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "acgs_assessments_select_by_role" ON public.acgs_assessments;

CREATE POLICY "acgs_assessments_select_by_role"
		...
		WHERE u.id = (SELECT auth.uid())
			...
			AND (
				...
				OR (
					...
					AND public.acgs_assessments.division_id = u.division_id
				)
			)
	)
);
...
-- ---------------------------------------------------------------------------
-- AOI RLS tightening: direct reads follow the same role/division model.
-- Server endpoints still use service role for managed writes.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Audit policies: wrap auth.uid() in SELECT for RLS performance advisor.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- ACGS audit trigger/RPC: explicit search_path and DB-derived actor context.
-- The RPC signature is kept for app compatibility, but p_user_email and
-- p_division_id are ignored for authorization/audit identity.
-- ---------------------------------------------------------------------------
...
BEGIN
	...
	INTO v_actor_email, v_actor_role, v_actor_division
	FROM public.users u
	...
	IF v_actor_role <> 'admin'
		AND NOT (
			v_actor_role = 'bpo'
			AND v_actor_division IS NOT NULL
			AND v_row_division = v_actor_division
		)
	THEN
...
-- Restrict sensitive functions from direct API callers. service_role keeps RPC access.
DO $$
DECLARE
	fn regprocedure;
BEGIN
...
-- ---------------------------------------------------------------------------
-- Storage: gcg-evidence remains private. App access must go through server
-- authorization and service-role signed URLs/removal.
-- ---------------------------------------------------------------------------
